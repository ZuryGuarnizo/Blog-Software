...
import tkinter as tk  # Biblioteca estándar para la interfaz gráfica de usuario
import customtkinter as ctk  # Biblioteca extendida de tkinter para una apariencia más moderna
...
def init_db():
    ...
    incluyendo las tablas de 'posts' y 'comments'.
    """
    conn = sqlite3.connect("blog.db")  # Conecta a la base de datos (se crea si no existe)
...
class BlogApp(ctk.CTk):
    """
    Clase principal de la aplicación que maneja la interfaz gráfica y las interacciones con el usuario.
    Hereda de ctk.CTk, una clase extendida de Tkinter.
    """
    def __init__(self):
    ...
    def clear_frame(self):
        ...
        if self.current_frame:
            self.current_frame.destroy()  # Destruye el frame actual para cambiar a otro
    ...
    def show_home(self):
        ...
        self.current_frame = ctk.CTkFrame(self, fg_color="#f8f9fa")  # Nuevo frame para la página de inicio
        ...
        posts = cursor.fetchall()  # Obtener todas las publicaciones
        conn.close()
        ...
        for post in posts:
            ...
            content_label = ctk.CTkLabel(post_frame, text=post[1], font=("Times New Roman", 12), text_color="#495057", wraplength=600, anchor="w")
    ...
    def create_post(self):
        ...
        if title and content:  # Validar si ambos campos están llenos
            conn = sqlite3.connect("blog.db")
            cursor = conn.cursor()
            ...
            tk.messagebox.showerror("Error", "Por favor, completa todos los campos.")  # Mostrar error si falta información

    def show_my_posts(self):
        """
        Muestra todas las publicaciones del usuario actual.
        """
        self.clear_frame()  # Limpiar el frame actual
        self.current_frame = ctk.CTkFrame(self, fg_color="#f8f9fa")
        self.current_frame.pack(fill="both", expand=True)
    ...
    def show_edit_post(self, post_id):
        """
        Muestra la pantalla para editar una publicación específica.
        """
        self.clear_frame()  # Limpiar el frame actual
        self.current_frame = ctk.CTkFrame(self, fg_color="#f8f9fa")
        self.current_frame.pack(fill="both", expand=True)
        ...
        # Título de la página de editar post
    ...
    def update_post(self, post_id):
        ...
        """
        title = self.edit_title_entry.get()  # Obtener el título editado
        content = self.edit_content_text.get("1.0", "end").strip()  # Obtener el contenido editado
        if title and content:  # Validar si ambos campos están llenos
            conn = sqlite3.connect("blog.db")
            cursor = conn.cursor()
            cursor.execute("UPDATE posts SET title = ?, content = ? WHERE id = ?", (title, content, post_id))  # Actualizar publicación
            conn.commit()
            conn.close()
    ...
    def delete_post(self, post_id):
        ...
        conn = sqlite3.connect("blog.db")
        cursor = conn.cursor()
        cursor.execute("DELETE FROM posts WHERE id = ?", (post_id,))  # Eliminar publicación
        conn.commit()
        conn.close()
        self.show_my_posts()  # Actualizar vista
...
if __name__ == "__main__":
    app = BlogApp()  # Crear instancia de la aplicación
    init_db()  # Inicializar la base de datos
    app.mainloop()  # Iniciar el bucle de eventos de la aplicación
